...
var _oldPos : Vector3
var time_til_disabled_timer := 0.0
var is_disabling := false

var update_timer := 0.0
const UPDATE_RATE := 0.05 # 20fps
...
func self_disable_process(_delta : float) -> void:
	time_til_disabled_timer -= _delta
	
	
	if time_til_disabled_timer <= 0.0:
		_trailEnabled = false
		time_til_disabled_timer = 0.0
		set_process(false)
		hide()
		return
...
func disable_trails(_time_til_disabled : float) -> void:
	time_til_disabled_timer = _time_til_disabled
	is_disabling = true
