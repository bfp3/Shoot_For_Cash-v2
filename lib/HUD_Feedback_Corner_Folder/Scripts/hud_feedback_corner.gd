...
func update_rocks_destroyed() -> void:
	...
	$Corner_Container/Current_total_score_tally/Coins_tracker/Coins_label.text = str(rocks_currently_active)
	main_score_GREY_flash()
	hud_missed_shot_feedback()
...
func player_has_reached_winning_score() -> void:
	...
	tween.tween_property(enemy_shots_astray_feedback, "modulate", Color.TRANSPARENT, 0.5).set_trans(Tween.TRANS_LINEAR)
	tween.parallel().tween_property(cage_damage_feedback, "modulate", Color.TRANSPARENT, 0.5).set_delay(0.25).set_trans(Tween.TRANS_LINEAR)
	#tween.tween_interval(0.5)
