...
## Level-script `music-start Opening_song` — play a named child under this Music node.
func script_start_music(song_name: String) -> void:
	var player := _find_music_player(song_name)
	if player == null:
		push_warning("Music: no child AudioStreamPlayer named '%s'" % song_name)
		return
	if player.stream == null:
		push_warning("Music: '%s' has no stream loaded yet" % player.name)
		return
	# Cancel any in-progress fade-out on this player.
	player.stop()
	if not default_volume_map.has(player):
		default_volume_map[player] = player.volume_db
	var target_vol: float = float(default_volume_map[player])
	if target_vol <= -79.0:
		target_vol = -35.0
...
## Level-script `music-stop Opening_song` — fade out over `fade_sec` then stop.
func script_stop_music(song_name: String, fade_sec: float = 3.0) -> void:
	var player := _find_music_player(song_name)
	if player == null:
		push_warning("Music: no child AudioStreamPlayer named '%s'" % song_name)
		return
	if not player.playing:
		return
	var dur := maxf(fade_sec, 0.05)
	var tween := create_tween().set_ease(Tween.EASE_IN)
	tween.tween_property(player, "volume_db", -80.0, dur)
	await tween.finished
	if is_instance_valid(player):
		player.stop()
		if default_volume_map.has(player):
			player.volume_db = float(default_volume_map[player])


func _find_music_player(song_name: String) -> AudioStreamPlayer:
	var exact := get_node_or_null(song_name)
	if exact is AudioStreamPlayer:
		return exact
	var want := song_name.strip_edges().to_lower()
	for child in get_children():
		if child is AudioStreamPlayer and String(child.name).to_lower() == want:
			return child
	return null
