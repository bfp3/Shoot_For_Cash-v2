...
func find_target_position() -> Vector3:
	var marker_node = get_tree().get_first_node_in_group("Bonus_round_markers")
	if marker_node == null:
...
func spawn_cherry_rigid() -> void:
	
	var cherry_amount = randi_range(1,3)
	for i in range(cherry_amount):
		var cherry_drop = preload("res://lib/Bonus_Round/Cherries_rigidbody.tscn").instantiate()
		get_tree().get_current_scene().add_child(cherry_drop)
		cherry_drop.global_position = global_position
...
func give_player_ammo() -> void:
	var ammo_up = get_tree().get_first_node_in_group("Ammo_management_system")
	if ammo_up:
		ammo_up.reserve_bullets += 1
