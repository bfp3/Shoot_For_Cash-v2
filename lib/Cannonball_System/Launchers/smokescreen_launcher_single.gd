...
const SMOKESCREEN_ROUND = preload('uid://cfc5u8dbb7fqq')
...
func send_smokescreen() -> void:
	var smoker = SMOKESCREEN_ROUND.instantiate()
	
	smoker.smokescreen = true
	smoker.slow_travel_time = launch_speed * 1.0
	smoker.fast_travel_time = launch_speed * 0.6
	smoker.travel_time = launch_speed
	smoker.arc_strength = arc_strength
	
	add_child(smoker)
	
	smoker.bomb_mesh.show()

	smoker.global_position = global_position
	
	smoker.target_launcher_fire("GREY")
	smoker.particles.amount = 100
	firing_sequence()
	launching_tween()
...
func firing_sequence() -> void:
	...
	if $launch_sound:
		$launch_sound.pitch_scale = randf_range(0.08,0.10)
		$launch_sound.volume_db = randf_range(-26.0,-30.0)
