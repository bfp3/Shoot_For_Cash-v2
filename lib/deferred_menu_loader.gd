...
const MENU_SCENES := {
	...
	"level_select": "res://ch/canvas_layers/start_menu_level_select.tscn",
...
func ensure_level_select() -> Control:
	return ensure("level_select") as Control
	...
	match key:
		...
		"difficulty_select":
			...
			if inst is Control:
				(inst as Control).mouse_filter = Control.MOUSE_FILTER_IGNORE
		"level_select":
			inst.name = "StartMenuLevelSelect"
			if inst is CanvasItem:
				(inst as CanvasItem).visible = false
				(inst as CanvasItem).z_index = 41
			if inst is Control:
