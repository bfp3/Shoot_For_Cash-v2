...
var parent: Popper_Behaviour = null

func start(_parent : CharacterBody3D) -> void:
	parent = _parent
	die()
...
func spawn_new_self() -> void:
	...
	for i in range(rand_number):
		...
		var SPOTTERS = preload('res://200_characters/02_enemies/Popper_Behaviour_Selector/Popper_Basic.tscn')
