...
@export var first_appearing_time_dur := 1.0
...
func jumping_shocked_tween() -> void:
	...
	target_rotation = Vector3(45, randf_range(-25,25), 45)
	
	var rigid_body_3d: RigidBody3D = get_tree().get_first_node_in_group('rock_rigid_body')
	rigid_body_copy = rigid_body_3d
	#var height = randf_range(3.0, 5.0)
	#jump_duration = 2.0  # total air time
	#jump_velocity = Vector3(0, (2 * height) / (jump_duration / 2), 0)
