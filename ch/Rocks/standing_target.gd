...
func _ready() -> void:
	...
	# Skip the INACTIVE state and any startup delay entirely - the orange
	# is visible and shootable the instant it spawns.
	make_ready_to_shoot()


func make_ready_to_shoot() -> void:
...
func update_active() -> void:
	# Ready immediately - the player can shoot this the instant it goes active.
	reset_stats()
	reset_rock_back_on()
	enable_collision()
...
func shake_camera() -> void:
	...
	if player_cam and player_cam.has_method("shake_camera_standing_target"):
		player_cam.shake_camera_standing_target()
...
func fly_straight_up() -> void:
	linear_velocity = Vector3.ZERO
	angular_velocity = Vector3.ZERO
	gravity_scale = 0.15
	linear_damp = 0.5
	apply_central_impulse(Vector3.UP * 25.0)
	apply_torque_impulse(Vector3.UP * 500.0)


func hit_by_player(damage : int, screen_offset : Vector2 = Vector2.ZERO) -> void:
	# Already been hit - ignore any follow-up shots while it's on its way up.
	if is_deactivated:
		return
