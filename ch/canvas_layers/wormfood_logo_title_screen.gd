...
func start():
	...
	copyright.scale = Vector2.ONE * 10.0
	...
	tween.tween_interval(1.0)
	tween.tween_property(copyright, "modulate:a", 1.0, 0.35)
	tween.parallel().tween_property(copyright, "scale", Vector2.ONE, 0.35)
	#tween.parallel().tween_callback($stampSFX.play).set_delay(0.32)
	tween.parallel().tween_callback($stampSFX22.play).set_delay(0.32)
	tween.tween_property(copyright, "scale", Vector2.ONE * 1.5, 0.1)
	tween.tween_property(copyright, "scale", Vector2.ONE, 0.05)
