...
signal add_strike()
signal balloon_strike_requested(world_position)
signal hazard_hit()
signal bonus_oranges()
signal all_white_compulsory_rocks_destroyed()
signal all_rocks_destroyed()
signal rocks_cleared_end_wave()
signal rest_balloon_shot()
signal checkpoint_shot() ## Deprecated alias — prefer rest_balloon_shot.
...
signal update_money()
signal continue_fee_changed(new_fee)
signal continue_reserve_broken()
