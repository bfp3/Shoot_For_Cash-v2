...
- Round modifiers on the sequence dict: `no_lives`, `bonus` (e.g. protect), `shuffle`, `difficulty` (`beginner` / `advanced` / `expert` pace table), `max_strikes`, `hold_out_ms`.
- Pace: `pace-slowest` / `slow` / `normal` / `fast` / `fastest` / `impossible` mid-round. Sets `aim_launch_gravity_scale` for every rock from that line on. Gravity numbers come from the range's `difficulty-beginner` (default) / `difficulty-advanced` / `difficulty-expert` table. (`rock-avoider` always uses gravity 1.0; `rock-stay` ignores pace and flies straight then hangs. Multi-cell: `rock-stay 1 a1 a8 c8 c1 a4 1` visits those cells then exits to splash; trailing `0`/omit hangs on the last cell.)
- Range `difficulty-*`: put under the `range` header (or inside that range). Only that range uses the matching pace table; the next range can pick another.
- `wait N`: clock delay of N milliseconds, then the next command (including `ammo`) even if rocks are still in the air. Bare `wait` / `wait-until-clear` still wait until live rocks / pineapples / balloon-rests are gone.
...
- `rock-bounce`: `$rock-bounce` mesh. No strike on miss. Hop side is automatic like rico (left of centre → right, right → left); `bounce_y` still aims the row. Health 2: each shot that leaves health hops and hangs; the last shot destroys it. `rock_bounce_on_timer` (default on) explodes it after 3s sitting without a shot; a hit resets that timer. While hopping it pops other rocks like rico. Does not block `wait` / `wait-until-clear`.
